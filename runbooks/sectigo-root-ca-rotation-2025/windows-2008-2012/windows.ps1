<# Datadog Agent v5 runbook — Windows Server 2008 R2 and 2012 R2 compatible.
   Requires PowerShell 2.0+ and .NET 3.5+.

   Differences from ../windows.ps1:
     1. ToUnixTimeSeconds() removed — added in .NET 4.6, absent on 2008 R2 (.NET 4.0)
        and 2012 R2 (.NET 4.5.1). Replaced with a manual epoch subtraction.
     2. Invoke-WebRequest replaced — cmdlet requires PS 3.0, absent on 2008 R2 default
        (PS 2.0). Primary downloader is now System.Net.WebClient (works from .NET 2.0).
        Invoke-WebRequest and BITS are kept as fallbacks.
     3. Get-Content -Raw replaced — switch requires PS 3.0. Replaced with
        [System.IO.File]::ReadAllText() which works from .NET 2.0.
     4. [Environment]::Is64BitOperatingSystem replaced — requires .NET 4.0, not present
        on PS 2.0 + .NET 3.5. Replaced with [IntPtr]::Size which works everywhere.
     5. TLS 1.2 forced via the numeric value 3072 instead of the named enum member
        [Net.SecurityProtocolType]::Tls12, which was added in .NET 4.5. On Windows
        2008 R2 without KB3140245 + SChannel registry fix, TLS 1.2 is unsupported at
        the OS level and the download will still fail — see the README for details.
     6. Adds -CertFile parameter for hosts that cannot reach raw.githubusercontent.com.
...
function Get-DdV5-CertPath {
    ...
    # [IntPtr]::Size -eq 8 works on PS 2.0 + .NET 2.0+.
    # [Environment]::Is64BitOperatingSystem requires .NET 4.0 and is not available
    # on Windows 2008 R2 with the default PS 2.0 / .NET 3.5 stack.
...
function Enable-Tls12 {
    ...
    try {
        ...
        # Wrap the -f format expression in its own parentheses so PowerShell does not
        # mistake -f for a Write-Warning parameter name in command-parse mode.
...
function Update-DatadogConfig {
    ...
    $raw = Get-Content -LiteralPath $ConfFile
    $lines = @()
    # On PS 2.0, Get-Content returns an array of strings (no -Raw switch needed here).
    foreach ($line in $raw) {
        if ($line -match '^\s*#?\s*use_curl_http_client\s*[:=].*$') { continue }
        $lines += $line
    }
    $lines += 'use_curl_http_client: true'
