#!/bin/bash
# Datadog Agent v5 runbook — Enterprise Linux (EL) 5 and 6 compatible.
# Covers: RHEL 5/6, CentOS 5/6, Oracle Linux 5/6.
#
# Differences from ../linux.sh (EL 7+ script):
#   1. Uses >/dev/null 2>&1 instead of &> (safer on bash 3.1 shipped with RHEL 5)
#   2. Replaces `truncate` with a portable sudo sh -c redirect (truncate is absent
#      from coreutils 5.97 shipped with RHEL 5 / CentOS 5 / OL 5)
#   3. Falls back to --insecure / --no-check-certificate if the system CA bundle
#      is too old to verify raw.githubusercontent.com (common on RHEL/CentOS 5)
#   4. Removes the journalctl check entirely — RHEL/CentOS/OL 5 and 6 use SysV
#      init and do not ship systemd/journald
...
detect_os_version() {
  if [ -f /etc/redhat-release ]; then
    ...
    if [ "$OS_MAJOR" != "5" ] && [ "$OS_MAJOR" != "6" ]; then
      echo "Warning: This script targets RHEL/CentOS/Oracle Linux 5 and 6." \
           "For version 7+, use the standard ../linux.sh instead."
    fi
  else
    echo "Warning: /etc/redhat-release not found; cannot confirm OS version."
  fi
}
...
# Download helper with a two-stage approach:
#   Stage 1 — standard TLS verification (preferred).
#   Stage 2 — skip TLS verification if stage 1 fails, which happens on RHEL/CentOS 5
#             because the system CA bundle (ca-bundle.crt) predates the DigiCert roots
#             used by GitHub. A clear warning is printed so the operator is informed.
#             The cert being retrieved is the Datadog application cert, NOT a system CA,
#             so the integrity risk is limited; verify_certificate() checks usability
#             against the Datadog endpoint afterwards.
download_url() {
  local url="$1"
  local outfile="$2"
...
restart_agent() {
  ...
  # RHEL/CentOS/OL 5 and 6 use SysV init; `service` is the correct interface.
...
test_connectivity_since_restart() {
  ...
  # No journalctl on RHEL/CentOS/OL 5 or 6 — systemd is not present.
